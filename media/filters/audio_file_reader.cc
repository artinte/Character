...
namespace media {
    ...
    int AudioFileReader::Read(std::vector<std::unique_ptr<AudioBus>>* decoded_audio_packets,
        ...
        int total_frames = 0;
        auto frame_ready_cb =
                base::BindRepeating(&AudioFileReader::OnNewFrame,
    ...
    int AudioFileReader::GetNumberOfFrames() const {
        return std::ceil(GetDuration().InSecondsF() * sample_rate());
    }
    ...
    bool AudioFileReader::OnNewFrame(int* total_frames,
            ...
            switch (bytes_per_sample) {
                ...
                case 4:
                    audio_bus->FromInterleaved<SignedInt32SampleTypeTraits>(
                            reinterpret_cast<const int32_t*>(frame->data[0]),
                            frames_read);
                    break;
                default:
                    LOG_ASSERT(false) << "Unsupported bytes per sample encountered: "
                                      << bytes_per_sample;
